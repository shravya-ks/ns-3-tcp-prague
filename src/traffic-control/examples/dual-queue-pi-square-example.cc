...
NS_LOG_COMPONENT_DEFINE ("DualQueuePiSquareExample");
...
{
  uint32_t qSize = StaticCast<DualQueuePiSquareQueueDisc> (queue)->GetQueueSize ();
...
{
  LogComponentEnable ("DualQueuePiSquareQueueDisc", LOG_LEVEL_INFO);

  std::string dualQueuePiSquareLinkDataRate = "1.5Mbps";
  std::string dualQueuePiSquareLinkDelay = "20ms";
  ...
  bool printDualQueuePiSquareStats = true;
  ...
  // DualQueuePiSquare params
  NS_LOG_INFO ("Set DualQueuePiSquare params");
  Config::SetDefault ("ns3::DualQueuePiSquareQueueDisc::Mode", StringValue ("QUEUE_DISC_MODE_PACKETS"));
  Config::SetDefault ("ns3::DualQueuePiSquareQueueDisc::MeanPktSize", UintegerValue (meanPktSize));
  Config::SetDefault ("ns3::DualQueuePiSquareQueueDisc::QueueLimit", UintegerValue (100));
  ...
  TrafficControlHelper tchDualQueuePiSquare;
  handle = tchDualQueuePiSquare.SetRootQueueDisc ("ns3::DualQueuePiSquareQueueDisc");
  tchDualQueuePiSquare.AddInternalQueues (handle, 2, "ns3::DropTailQueue", "MaxPackets", UintegerValue (1000));
  ...
  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue (dualQueuePiSquareLinkDataRate));
  p2p.SetChannelAttribute ("Delay", StringValue (dualQueuePiSquareLinkDelay));
  devn2n3 = p2p.Install (n2n3);
  // only backbone link has DualQueuePiSquare queue disc
  queueDiscs = tchDualQueuePiSquare.Install (devn2n3);
  ...
  if (writePcap)
    {
      ...
      stmp << pathOut << "/dual-queue-pi-square";
  ...
  if (writeForPlot)
    {
      filePlotQueueDisc << pathOut << "/" << "dual-queue-pi-square-queue-disc.plotme";
      filePlotQueueDiscAvg << pathOut << "/" << "dual-queue-pi-square-queue-disc_avg.plotme";
  ...
  DualQueuePiSquareQueueDisc::Stats st = StaticCast<DualQueuePiSquareQueueDisc> (queueDiscs.Get (0))->GetStats ();
  ...
  if (printDualQueuePiSquareStats)
    {
      std::cout << "*** DualQueuePiSquare stats from Node 2 queue ***" << std::endl;
