...
class DualQCoupledPiSquareQueueDiscTestCase : public TestCase
{
public:
  DualQCoupledPiSquareQueueDiscTestCase ();
  virtual void DoRun (void);
private:
  void Enqueue (Ptr<DualQCoupledPiSquareQueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType);
  void EnqueueWithDelay (Ptr<DualQCoupledPiSquareQueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType);
  void Dequeue (Ptr<DualQCoupledPiSquareQueueDisc> queue, uint32_t nPkt);
  void DequeueWithDelay (Ptr<DualQCoupledPiSquareQueueDisc> queue, double delay, uint32_t nPkt);
...
void
DualQCoupledPiSquareQueueDiscTestCase::RunPiSquareTest (StringValue mode)
{
  uint32_t pktSize = 0;
  ...
  uint32_t qSize = 50;
  Ptr<DualQCoupledPiSquareQueueDisc> queue = CreateObject<DualQCoupledPiSquareQueueDisc> ();
  ...
  if (queue->GetMode () == DualQCoupledPiSquareQueueDisc::QUEUE_DISC_MODE_BYTES)
    {
...
void
DualQCoupledPiSquareQueueDiscTestCase::EnqueueWithDelay (Ptr<DualQCoupledPiSquareQueueDisc> queue, uint32_t size, uint32_t nPkt, StringValue trafficType)
{
  Address dest;
...
void
DualQCoupledPiSquareQueueDiscTestCase::Dequeue (Ptr<DualQCoupledPiSquareQueueDisc> queue, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
...
void
DualQCoupledPiSquareQueueDiscTestCase::DequeueWithDelay (Ptr<DualQCoupledPiSquareQueueDisc> queue, double delay, uint32_t nPkt)
{
  for (uint32_t i = 0; i < nPkt; i++)
    {
      Simulator::Schedule (Time (Seconds ((i + 1) * delay)), &DualQCoupledPiSquareQueueDiscTestCase::Dequeue, this, queue, 1);
    }
}

void
DualQCoupledPiSquareQueueDiscTestCase::DoRun (void)
{
...
static class DualQCoupledPiSquareQueueDiscTestSuite : public TestSuite
{
public:
